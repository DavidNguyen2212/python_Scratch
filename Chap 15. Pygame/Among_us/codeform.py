...
os.chdir(r"C:\Users\DELL\Documents\Python3\Chap 15. Pygame\Among_us\images")
...
def draw():
    ...
    if game_over:
        display_message("Game over!", "Press space bar to play again")
    elif game_complete:
        display_message("You won", "Press space bar to play again")
    else:
        for im in impostors:
            im.draw()
...
def update():
    global impostors, current_level, game_over, game_complete
    if len(impostors) == 0:     # chưa có impostor nào được tạo ra
...
def make_impostors(number_of_impostors):
    ...
    layout_impostors(new_impostors)  # đặt ra màn hình
    animate_impostors(new_impostors) # làm nó rơi tự do
...
def on_mouse_down(pos):
    ...
    for impostor in impostors:
        if impostor.collidepoint(pos):      # click vào imposter tại position
            if "red" in impostor.image:
                red_impostor_click()        # màu đỏ thì gọi hàm click
            else:
                handle_game_over()          # nếu không thua cuộc
